...
func bfs2() int {
	...
	for curr[0] < 51 {
		...
		next = next[1:]
		for i := -1; i < 2; i++ {
			if !isWall(x+i, y) && !hasVisited(visited, []int{x + i, y}) && x+i >= 0 && y >= 0 {
				visited = append(visited, []int{x + i, y})
				next = append(next, []int{curr[0] + 1, x + i, y})
			}
		}
		for i := -1; i < 2; i++ {
			if !isWall(x, y+i) && !hasVisited(visited, []int{x, y + i}) && x >= 0 && y+i >= 0 {
...
func hasVisited(v [][]int, i []int) bool {
	for _, pair := range v {
