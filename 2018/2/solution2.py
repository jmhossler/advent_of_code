def get_checksum(box_ids):
    return (len([box_id for box_id in box_ids if contains_matching(2, box_id)])
            * len([box_id for box_id in box_ids if contains_matching(3, box_id)]))
...
def get_common_letters(box_ids):
    for i in range(0, len(box_ids)):
        for j in range(i, len(box_ids)):
            if differences(box_ids[i], box_ids[j]) == 1:
                return remove_difference(box_ids[i], box_ids[j])
    return None
